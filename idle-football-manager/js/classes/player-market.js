class PlayerMarket {
    ...
    refresh() {
        ...
        for (let i = 0; i < Math.floor(Math.random() * 10) + 15; i++) {
            ...
            let price = Decimal.min(Decimal.max(new Decimal(1e2), game.money.div(10)), Decimal.pow(priceBase, GeneratorUtils.getNormRank(rank, game.country)).mul(250)).mul(1 + 3 * fact ** pow * (0.8 + 0.4 * Math.random()));
