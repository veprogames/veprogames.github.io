    ...
    constructor()
    {
        ...
        this.powerUpgrades = [
            new MetaDynamicLayerUpgrade("Increase the Resource Power",
                ...
                level => new Decimal(1),
                level => Decimal.pow(1.2, level).pow(this.getResourcePowererBoost()), {
            ...
            new MetaDynamicLayerUpgrade("Increase the Resource Power",
                ...
                level => new Decimal(1),
                level => Decimal.pow(1.1, level).pow(this.getResourcePowererBoost()), {
                    getEffectDisplay: effectDisplayTemplates.numberStandard(2, "^")
                }),
            new MetaDynamicLayerUpgrade("Increase the Resource Power",
                level => Utils.createValueDilation(level.mul(2e12).mul(Decimal.pow(10, Decimal.max(0, level.sub(1)))), 0.001).floor().add(1e12),
                level => new Decimal(1),
                level => Decimal.pow(1.05, level).pow(this.getResourcePowererBoost()), {
                    getEffectDisplay: effectDisplayTemplates.numberStandard(2, "^")
                })
        ];
    }
